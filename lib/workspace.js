 ...
 */
function copyTreeMissing(from, to) {
 ...
 *
 * 规则：
 *   · 留空                → `<workspace>/staging/inbox`（推荐）
 *   · 含 `{workspace}`    → 替换成工作区根
 *   · 绝对路径            → 原样使用
 *   · 其它相对路径        → **相对工作区**解析
 *
 * 注意：这里**不**支持 `{top}`（库内顶层目录名）。收件目录是工作区里的暂存区，
 * 与 Obsidian 库的目录结构无关；把 `{top}` 写进来会得到一个名为 `{top}` 的字面
 * 目录。库内的落点由「存储路径规则」决定，两者是两回事。
 *
 * @param {object} settings - 已解析的设置值。
...
export function resolveInboxDir(settings, workspace) {
  ...
  if (!raw) return path.join(workspace, 'staging', 'inbox');
  if (/\{(?!workspace\})[a-zA-Z]+\}/.test(raw)) {
    // 写了不支持的占位符（最常见的是 {top}）：与其产出名为 `{top}` 的字面目录，
    // 不如直接说到底该怎么写。
    throw new Error(
      `媒体收件目录里的占位符不受支持：${raw}。`
      + '只支持 {workspace}（工作区根），或直接留空用 {workspace}/staging/inbox；'
      + '库内的目录结构由「存储路径规则」决定，与本项无关。',
    );
  }
  const expanded = raw.replace(/\{workspace\}/g, workspace);
  return path.isAbsolute(expanded) ? path.normalize(expanded) : path.resolve(workspace, expanded);
}
...
export function materialize(settings) {
  ...
  copyTreeMissing(TEMPLATE_PIPELINE, workspace);
  for (const sub of ['staging/converted', 'staging/formatted', 'staging/inbox', 'logs/runs', 'logs/state']) {
  ...
  // 3. 收件目录（媒体落盘位置）
  //
  // 早期实现直接把该项当字符串 `path.join()` 用，于是两个坑：
  //   · 写相对路径 → 静默落到 DSH 进程的 cwd（不是工作区），媒体文件找不着；
  //   · 写 `{top}` 之类的占位符 → 原样当目录名，产出 `{top}` 这种字面目录。
  // 现在显式支持 `{workspace}` 占位符，且相对路径一律相对工作区解析。
