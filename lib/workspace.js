 ...
 *      模板源**，每次物化都同步成可写工作区：新增 + 覆盖更新 + 清理已删除的
 *      模板文件 —— 升级后必须跑新脚本，只「补齐缺失」会让用户继续跑旧逻辑）；
...
/**
 * 记录上一次落位进工作区的模板文件清单（相对路径），供升级时精确清理。
 *
 * 放在工作区根下的隐藏文件里，不进 `config/`（那是脚本的配置目录）。
 */
const TEMPLATE_MANIFEST = '.wecom-obsidian-template.json';

/**
 * 用户数据顶层目录：模板同步**永不**读取、复制或删除。
 *
 * 采集账本（`logs/state/processed-urls.jsonl`）与中间产物（`staging/`）是用户的
 * 去重依据，不是插件模板。真实踩过：插件包 `pipeline/` 被采集会话同时当作工作区
 * 使用后，模板树里出现了真实账本 —— 于是它既会被同步进新工作区（覆盖库里的账本），
 * 又可能因为「这一版模板里没有」被清理掉。
 */
const USER_DATA_TOP = new Set(['logs', 'staging']);

/** 递归列出一个模板目录里的全部文件（相对路径，POSIX 分隔符）。 */
function listTree(from, prefix = '', excludeTop) {
  const out = [];
  ...
  for (const entry of fs.readdirSync(from, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) out.push(...listTree(src, rel, excludeTop));
    else out.push(rel);
  }
  return out;
}

/** 内容相同则不动，避免每次设置变更都重写整棵模板树（也会打乱 mtime）。 */
function copyIfChanged(src, dst) {
  const source = fs.readFileSync(src);
  try {
    if (fs.readFileSync(dst).equals(source)) return false;
  } catch { /* 目标不存在或不可读：按需要写入处理 */ }
  fs.mkdirSync(path.dirname(dst), { recursive: true });
  fs.writeFileSync(dst, source, { mode: fs.statSync(src).mode & 0o777 });
  return true;
}

/**
 * 把模板树**同步**进可写工作区：新增 + 覆盖更新 + 清理已删除的模板文件。
 *
 * 为什么不是「只补齐缺失」（历史实现）：`pipeline/` 在插件包里是**只读模板源**，
 * 工作区那份是它的派生物。只补齐会让升级后的用户继续跑**旧脚本** —— 拉了修复、
 * 重启了 DSH，入库逻辑却还是上一版（真实踩过：改了代码但库里行为没变）。
 *
 * 清理只针对**上一次清单里记录过、这一版已不存在**的模板文件（例如脚本改名），
 * 不会碰用户数据：`staging/`、`logs/` 以及用户自己放的文件都不在清单里。
 * 这两条都是**硬约束**：插件包里的 `pipeline/` 在本机可能被同时当作运行时工作区
 * 使用（历史遗留的采集会话会往那里写账本/暂存），模板同步绝不能把它们当模板
 * 复制进工作区、更不能按旧清单把它们删掉（会把用户账本清空）。
 *
 * @param {string} from - 模板源目录。
 * @param {string} to - 工作区目录。
 * @param {Set<string>} [excludeTop] - 需要跳过的顶层条目名（目前是 skills：唯一副本在插件包里）。
 * @returns {{copied: number, removed: number}} 本次变更计数（便于日志/测试）。
 */
function syncTemplateTree(from, to, excludeTop) {
  if (!fs.existsSync(from)) return { copied: 0, removed: 0 };
  fs.mkdirSync(to, { recursive: true });
  const wanted = new Set(listTree(from, '', excludeTop));

  let copied = 0;
  for (const rel of wanted) {
    if (copyIfChanged(path.join(from, rel), path.join(to, rel))) copied += 1;
  }

  const manifestPath = path.join(to, TEMPLATE_MANIFEST);
  let previous = [];
  try {
    const parsed = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
    previous = Array.isArray(parsed && parsed.files) ? parsed.files : [];
  } catch { /* 首次运行 / 清单损坏：当作没有历史清单 */ }

  let removed = 0;
  for (const rel of previous) {
    if (typeof rel !== 'string' || !rel || wanted.has(rel) || rel.includes('..')) continue;
    if (USER_DATA_TOP.has(rel.split('/')[0])) continue;
    try {
      fs.rmSync(path.join(to, rel));
      removed += 1;
    } catch { /* 已经不在了 */ }
  }
  ...
  try {
    ...
    console.warn(`[wecom-obsidian] 模板清单写入失败 ${manifestPath}: ${String((error && error.message) || error)}`);
  }
...
export function storeSection(settings) {
  ...
  return {
    ...
    // 机器可读的同名冲突策略：脚本据此决定「递增 -2/-3」还是「直接失败」。
    // `naming.conflict` 只是给人看的描述，不能当判定依据。
    conflict_suffix: store.conflictSuffix !== false,
    naming: {
 ...
 *
 * `convert.timeout_sec` 是「单篇转换超时」的落点：转换脚本由收藏会话里的 agent
 * 以 bash 调用，宿主无法掐单个进程，所以把上限物化进配置，由
 * `scripts/convert/_common.run_guarded()` 在每个转换脚本入口做**墙钟**兜底。
 * 整条链路的超时另有 `pipeline.pipelineTimeoutSec`（宿主侧回合上限）。
 *
 * @param {object} settings - 已解析的设置值。
 * @returns {object} 完整配置对象。
 */
export function pipelineConfig(settings) {
  const pipeline = (settings && settings.pipeline) || {};
  const convertTimeoutSec = Number(pipeline.convertTimeoutSec) > 0 ? Number(pipeline.convertTimeoutSec) : 180;
  return {
    说明: '【wecom-obsidian 插件生成】由设置页物化，勿手改。',
    webp_to_png: true,
    convert: {
      timeout_sec: convertTimeoutSec,
...
export function materialize(settings) {
  ...
  // 1. 骨架落位：模板源 → 可写工作区（新增 + 覆盖更新 + 清理已删除的模板文件）。
  //    覆盖更新是必须的：升级后必须跑新脚本，否则「拉了修复、重启了，行为没变」。
  //    用户数据（staging/、logs/、自建文件）不在模板清单里，永远不动；
  //    `config/` 也跳过 —— 它是插件**生成**的（下面 writeJsonAtomic 全量重写），
  //    模板里那份只是仓库内独立运行时的默认值，同步过来只会被立刻覆盖；
  //    `tests/` 是仓库里的回归测试，运行时工作区用不上（路径假设也不成立）。
  const sync = syncTemplateTree(
    TEMPLATE_PIPELINE, workspace,
    new Set(['skills', 'config', 'tests', ...USER_DATA_TOP]));
  ...
  return {
    ...
    // 模板同步计数：升级后用它确认「工作区脚本真的被更新了」（copied>0）。
    template: sync,
    obsidian,
    pipeline,
    runtime,
