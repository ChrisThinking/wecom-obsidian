 ...
 * 把任意标签归一成安全的文件名片段。
 *
 * 注意：中文标签会被整体压成下划线（`示例机器人0` → `_0_`），多个中文标签因此
...
export class WecomBot {
   ...
   * @param {boolean} [options.mediaEnabled] - 是否下载图片/文件到收件目录（设置页每台机器人可关）。
   * @param {boolean} [options.replyAck] - 收到消息是否先回一条「已收到」回执（设置页全局开关）。
  ...
  constructor(options) {
    ...
    /**
     * 设置页「图片/文件下载」与「收到消息先回执」两个开关的落点。
     * 默认 true（与 schema 默认值一致）；显式 false 才关闭。
     */
    this.mediaEnabled = opts.mediaEnabled !== false;
    this.replyAck = opts.replyAck !== false;
  ...
  async handleInbound(frame, payload) {
    // `replyAck=false` 时不发「已收到」回执，但仍要为最终回复开一个流式气泡。
    //
    // 必须用 `ackStream()` 的**返回值**，不能回头看实例上的 `this.lastStreamId`：
    // 两条消息并发时，A 的 ack 还挂在 await 上，B 的 ack 已经把 lastStreamId
    // 覆盖成自己的 —— A 恢复后拿到的是 B 的 id，最终回复被发进 B 的气泡
    // （真实踩过：两个人同时发消息，答案串了）。
    const sid = this.replyAck
      ? await this.ackStream(frame, '✅ 已收到，正在处理，请稍候…')
      : this.generateReqId('stream');
    let answer = null;
    let reason = 'ok';
    try {
  ...
  async handleMedia(frame, kind, label) {
    ...
    // 设置页「图片/文件下载」关闭时，不下载、不落盘，只回一条明确说明
    // （让用户知道是配置关掉了，而不是机器人坏了）。
    if (!this.mediaEnabled) {
      const note = `ℹ️ 本机器人已关闭「图片/文件下载」，${label}未保存。`
        + '需要保存媒体请在设置页打开该机器人的「图片/文件下载」。';
      this.log('media.disabled', { kind });
