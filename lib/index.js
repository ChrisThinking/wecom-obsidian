...
import { consumeBotOps as consumeBotOpsFromQueue } from './bot-ops.js';
...
function applyOnce(ctx, log) {
  ...
  const refreshPaths = (settings) => {
    ...
    log('workspace.ready', {
      ...
      // 媒体收件目录：现在默认落在库内（`<库根>/<顶层目录>/attachments`），
      // 打到日志里，排障时不必再猜「媒体到底存哪了」。
      mediaInbox: paths.inbox,
      bots: Array.isArray(settings.bots) ? settings.bots.length : 0,
      // 模板同步计数：升级后 copied>0 说明工作区里的脚本确实被更新过
      // （只补齐缺失的旧实现会让这里永远是 0，跑的还是旧逻辑）。
      templateSync: paths.template || null,
    });
    return paths;
  };
   ...
   * 读**未脱敏**的原始 user 段。
   *
   * `scope.get()` 只给解析后的值，拿不到「用户实际写了什么」；而结构变更必须
   * 在原始段上做，才能把没被浏览器读到的 secret 原样带过去。`describe()` 不带
   * `redactSecrets` 时会把 `user`（原始段）一并给出 —— 只在宿主侧调用。
   */
  const readUserSection = () => {
    try {
      const rows = typeof ctx.settings.describe === 'function' ? ctx.settings.describe() : [];
      const row = Array.isArray(rows) ? rows.find((item) => item && item.ns === SETTINGS_NS) : undefined;
      return row && row.user && typeof row.user === 'object' && !Array.isArray(row.user) ? row.user : {};
    } catch (error) {
      log('settings.user.read.failed', { message: String((error && error.message) || error) });
      return {};
    }
  };

  /** 已消费的命令 nonce：同一条命令重复出现在队列里时不重复执行。 */
  const botOpNonce = { value: '' };

  /**
   * 消费配置页提交的机器人结构变更意图，然后精确写回原始段。
   *
   * 具体逻辑在 `lib/bot-ops.js`（纯函数 + 注入读写通道），这样同一段代码既能被
   * 宿主使用，也能在测试里对着**真实设置服务**跑回归。
   *
   * @param {object} settings - 当前已解析设置。
   * @returns {Promise<boolean>} 是否消费了至少一条命令（true 时调用方不必再 reconcile，
   *   `replace()` 的提交会再触发一次设置变更）。
   */
  const consumeBotOps = (settings) => consumeBotOpsFromQueue({
    readUserSection,
    replace: (section) => scope.replace(section),
    isDisposed: () => disposed,
    lastNonceRef: botOpNonce,
  }, settings, log);
  ...
  const startBot = (label, slug, botConfig, settings) => {
    ...
    const instance = new WecomBot({
      ...
      // 每台机器人的「图片/文件下载」开关，以及全局的「收到消息先回执」。
      // 设置页提供了它们，就必须真的落到处理路径上（否则页面承诺与行为不一致）。
      mediaEnabled: botConfig.mediaEnabled !== false,
      replyAck: settings.replyAck !== false,
  ...
  /**
   * 一次设置变更的完整处理：先消费机器人结构变更意图，再全量对齐实例。
   *
   * 消费成功后**不再** reconcile：`replace()` 的提交会再触发一次设置变更，
   * 那次带着最终列表收敛就够；否则每次增删都会把全部机器人无谓重连两遍。
   */
  const onSettingsChanged = async () => {
    try {
      if (await consumeBotOps(readSettings())) return;
    } catch (error) {
      log('botOps.consume.failed', { message: String((error && error.message) || error) });
    }
    reconcile();
  };
  ...
  ctx.effect(() => () => {
    disposed = true;
    generation += 1;
    // 复位进程级单例守卫：live reload（profile patch 层热更新）会 dispose 再
    // apply，但模块实例仍在缓存里 —— 不复位就会让第二次 apply() 直接短路，
    // 插件静默不装载（设置页显示 namespace 不存在）。
    applied = false;
    const previous = running;
    running = [];
    void (async () => {
  ...
  // 首次启动：同样先消费可能残留的命令（例如宿主在浏览器提交后重启过）。
  void onSettingsChanged();
