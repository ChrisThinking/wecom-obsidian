...
/**
 * 收集现有机器人已占用的展示名与会话 id。
 *
 * 会话 id 必须唯一：两条会话线（普通对话 / 收藏）都按 sessionId 复用 agent，
 * 撞车等于两台机器人共享同一段上下文（用户真实踩到过）。
 *
 * @param {Array} bots - 现有机器人（原始段，字段可能不全）。
 * @returns {{labels: Set<string>, sessions: Set<string>}}
 */
function usedIdentities(bots) {
  ...
  for (const item of Array.isArray(bots) ? bots : []) {
    ...
    if (bot.sessionId) sessions.add(String(bot.sessionId));
    if (bot.collectorSessionId) sessions.add(String(bot.collectorSessionId));
  }
  return { labels, sessions };
}

/**
 * 为「新增机器人」挑一个**未被占用**的序号。
 *
 * 为什么不能用 `bots.length + 1`：删除中间一台后数组长度与现有编号就错位了 ——
 * 三台（机器人1/2/3）删掉第二台剩两台，`length + 1` 又是 3 → 新增出第二台
 * 「机器人3」，`sessionId=wecom-3` 与既有的完全相同，两台机器人共用一段上下文。
 *
 * 取「最小的空闲序号」既避免撞车，也让编号回到被删掉的位置（用户直觉）。
 * 最多考察 `bots.length + 1` 个候选：N 台机器人最多占用 N 个名字，必然有空位。
 *
 * @param {Array} bots - 现有机器人。
 * @returns {number} 可用的序号（从 1 开始）。
 */
export function nextBotIndex(bots) {
  const list = Array.isArray(bots) ? bots : [];
  const { labels, sessions } = usedIdentities(list);
  const limit = list.length + 2;
  for (let n = 1; n <= limit; n += 1) {
    const candidate = defaultBotConfig(n);
    if (!labels.has(candidate.label)
      && !sessions.has(candidate.sessionId)
      && !sessions.has(candidate.collectorSessionId)) {
      return n;
    }
  }
  return limit + 1;
