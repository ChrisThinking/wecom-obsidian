...
| 28 | 小红书收藏突然全部失败：`RuntimeError: noteDetailMap 为空`，重试 3 次（含带 `xsec_token` 直链）结果一致 | 小红书改了页面投放：**桌面 UA** 请求笔记页一律 302 到 `/login?redirectPath=…`（登录墙页有 `__INITIAL_STATE__` 但没有 `noteDetailMap`）；移动端 UA 仍能拿到分享页，但那代页面把笔记放在 `noteData.data.noteData`、图片字段是 `url`/`infoList[].url`（旧实现只认 `note.noteDetailMap` + `urlDefault`）。只换 UA 或只改解析都仍然失败 | `xhs_convert.py`：取页改为**移动端 UA 优先**、落地 `/login` 即换 UA、全败报「登录墙」；`parse_note()` 兼容两代结构，`image_url()` 兼容三种图片字段，作者兼容 `nickName`/`nickname` → `pipeline/tests/test_xhs_page_variants.py`（真实短链 `xhslink.cn/o/1qG1J8J6lI8` 已 10/10 图入库） |
| 29 | 模板同步会**碰用户账本**：真实账本被同步进新工作区（覆盖库里去重记录），甚至可能按旧清单被删掉 | 插件包 `pipeline/` 在本机被采集会话同时当作运行时工作区用，账本/暂存因此出现在模板树里；`syncTemplateTree()` 的排除表只写了 `skills/config/tests`，清理循环又不区分来源 | 显式 `USER_DATA_TOP = {logs, staging}`：既不列入模板清单，也**永不按清单清理**（旧清单登记过也不删）；新工作区骨架仍由 `materialize` 自己建目录 → `tests/workspace-sync.test.mjs`（账本/清理两条断言） |
